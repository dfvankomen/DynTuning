...
struct FunctorKernel_YYDeriv2D_Host
{
    ...
    KOKKOS_FUNCTION void operator()(ViewsTuple views, const Index i, const Index j) const
    {
        ...
        if (i > 2 && i < U.extent(0) - 2 && j > 2 && j < U.extent(1) - 2)
        {
            ...
            dUdy(i, j) = (-U(i, j - 2) + 16.0 * U(i, j - 2) - 30.0 * U(i, j) + 16.0 * U(i, j + 1) -
                          U(i, j + 2)) /
                         (12.0 * h);
        }
#if 0
        if (i == 2)
        {
            // left edge
            dUdy(i, j) =
              (2.0 * U(i, j) - 5.0 * U(i, j + 1) + 4.0 * U(i, j + 2) - U(i, j + 3)) / (h);
        }
        else if (i == 3)
        {
            // one inside left edge
            dUdy(i, j) = (u(i, j - 1) + 2.0 * u(i, j) + u(i, j + 1)) / (h);
        }
        else if (i == U.extent(0) - 4)
        {
            // one inside right edge
            dUdx(i, j) = (u(i, j - 1) + 2.0 * u(i, j) + u(i, j + 1)) / (h);
        }
...
struct FunctorKernel_YYDeriv2D_Device
{
    template<typename ViewsTuple, typename Index>
    KOKKOS_FUNCTION void operator()(ViewsTuple views, const Index i, const Index j) const
    {
...
inline auto KernelYYDeriv2D(KernelOptions& options, ParameterTypes&... data_views)
{
